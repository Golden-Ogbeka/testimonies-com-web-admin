...
import type { AdminLoginRequest, AdminProfile, AdminVerifyOtpRequest } from "../types/auth";
...
export const AdminAuthApi = {
  ...
  resendOtp(email: string) {
    return appAxios.post<ApiSuccessResponse<unknown>>("/admin/auth/resend-otp", {
      email,
    });
  },

  requestPasswordReset(email: string) {
    return appAxios.post<ApiSuccessResponse<unknown>>("/admin/auth/reset-password", {
      email,
    });
  },
  ...
  updateProfile(payload: Partial<Pick<AdminProfile, "firstName" | "lastName" | "phoneNumber">>) {
